  ...
  return new Promise((resolve, reject) => {
    ...
    request.on("data", (chunk) => {
      ...
      if (body.length > 18_000_000) {
        reject(new Error("Images are too large. Try fewer or smaller photos."));
        request.destroy();
      }
    });
...
async function analyzeGroceries(request, response) {
  if (!OPENAI_API_KEY) {
    sendJson(response, 500, {
      error: "Missing OPENAI_API_KEY. Start the server with OPENAI_API_KEY=your_key npm start."
    });
    return;
  }
  ...
  try {
    ...
    sendJson(response, 400, { error: error.message || "Could not read the uploaded images." });
    return;
  }
  ...
  const content = [
    {
      type: "input_text",
      text: [
        "Carefully and patiently analyze these fridge, freezer, and pantry photos against the fixed grocery list.",
        ...
        "Also recommend up to 5 additional healthy, nutrient-dense grocery items that are not already on the fixed grocery list and are not visibly stocked in the photos. Favor practical fridge/freezer/pantry staples such as leafy greens, berries, beans, lentils, tofu, salmon, nuts, seeds, kefir, or similar whole foods.",
        "Only judge the fixed grocery list. Do not add other groceries.",
        `Fixed grocery list: ${groceryList.join(", ")}`
      ].join("\n")
    },
    ...images.map((image) => ({
      type: "input_image",
...
const server = createServer(async (request, response) => {
  if (request.method === "POST" && request.url === "/api/analyze") {
    await analyzeGroceries(request, response);
    return;
  }
...
server.listen(PORT, () => {
  console.log(`Groceries is running at http://127.0.0.1:${PORT}`);
});
